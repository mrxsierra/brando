...
### Step 1: Initialize Configuration Wizard
...
### Step 2: Build Candidates & DNS Verification
Generate candidates and run fast local calculations along with quick DNS-only domain lookups:
```bash
brando build --limit 100
```
- **What it does**: Combines prefixes/suffixes, runs typographic ratio math, calculates Chaldean and Pythagorean destiny numbers, and verifies whether `.com`/`.co`/`.io`/`.ai` domains resolve.
- **Performance**: Skips slow social media handle checks by default to keep execution time under a few seconds.
...
- **Why**: Runs HTTP requests lazily on a small pool to prevent rate limits and IP bans.
...
```
- **What it does**: Outputs clickable links to query trademark databases (USPTO TESS, WIPO Madrid Protocol), Google search, and dictionary definitions.
